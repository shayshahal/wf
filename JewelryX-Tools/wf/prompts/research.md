...
**Budget: 15 of your own tool calls**, plus the sub-agents below. When it is gone, write what
...
4. Write the repro yourself (below) and run it once. It must be red on this checkout.
...
## As-is
<call stack, JewelryX-Tools/wf/process/CALL-STACK-FORMAT.md, no markers; every hop file:line;
...
- `pnpm wf status` prints the named URLs. Seed users and fixed ids:
  `docs/agents/seed.md`. Login is password + OTP; `OTP_DEV_EXPOSE=true` returns the code.
